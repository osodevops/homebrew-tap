...
unless ARGV.length == 4
  abort "usage: update-zendesk-cli-formula.rb TAG REPOSITORY CHECKSUMS FORMULA"
end
...
unless repository == "osodevops/zendesk-cli"
...
targets.each do |target|
  ...
  pattern = %r{^(\s*)url "https://github\.com/osodevops/zendesk-cli/releases/download/[^/"]+/zdk-[^/"]+-#{Regexp.escape(target)}\.tar\.gz"\n\1sha256 "[0-9a-f]{64}"$}
